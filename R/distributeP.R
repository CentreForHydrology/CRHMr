...
#' 
#' @description Distributes total precipitation (often daily) evenly over all of the applicable intervals. The function has 2 uses. The primary use is to distribute total precipitation when the precipitation time step is greater than the time step of the obs dataframe. For example, there may be 3-hour total precipitation within an obs dataframe holding hourly values for all other variables. The secondary use is to disaggregate precipitation to a shorter time step. Normally this can be done inside CRHM by putting the data in a separate obs file. However, if you want to do this in this function, you must specify the new time step.
#' @param obs Required. The \pkg{CRHMr} data frame of obs values.
#' @param p.cols Optional. The number of the column(s) to be used. If not specified, the column will be guessed from the variable name. Note that the variable name MUST include the letter 'p', even if the number is specified.
#' @param timestep Optional. The new time step in hours. If zero (the default) the time step is not used. The time step is only used when disaggregating precipitation to a shorter time step. Note that the specified time step must be shorter than that used in the original data frame.
#' @param quiet Optional. Suppresses display of messages, except for errors. If you are calling this function in an \R script, you will usually leave \code{quiet=TRUE} (i.e. the default). If you are working interactively, you will probably want to set \code{quiet=FALSE}.
#' @param logfile Optional. Name of the file to be used for logging the action. Normally not used.
#' @return If sucessful, returns a modified version of the obs data frame. If no time step is specified, the missing precipitation values in the specified data frame are replaced by average precipitation values. If a time step is specified, a new data frame containing ONLY the datetime and the distributed precipitation will be returned. If unsuccessful, returns the value \code{FALSE}.
#' @author Kevin Shook
...
distributeP <- function(obs, p.cols=0, timestep=0, quiet=TRUE, logfile=''){
  ...
  if (timestep <= 0){
    ...
    action <- 'downscaling'
    if (dt < timestep){
      cat('Error: specified time step > data time step\n')
      return(FALSE)
    }
    # generate time synthetic series from beginning to end
    dt <- timestep
  ...
  comment <- paste('distributedP precip_dataframe:', obsName, 
                   ' Variables:', stringr::str_c(names(output.data)[-1], collapse=','),
                   sep='')  
