...
#'
#' @description Distributes mean values to a shorter time interval. The missing datetimes are inserted
#' and then the values are repeated. This function is typically used to downscale obs values such as
#' t, ea, and u.
#' @param obs Required. The \pkg{CRHMr} data frame of obs values.
#' @param obsCols Optional. A vector containing the columns to be imputed in the obs data frame, not
#' including the datetime. The default \option{all} specifies all columns.
#' @param timeStep Required. The time step (in hours) for the interpolated values. This value must be
#' smaller than the time step in the original time series.
...
#'
#' @return If successful, returns a dataframe of the selected columns interpolated to the specified time
#' step. If unsuccessful, returns an error.
#' @author Kevin Shook
...
distributeMean <- function(obs,  obsCols='all', timeStep=0, maxLength=5,
  ...
  if (timeStep >= dt) {
    stop("Time step must be smaller than in the original data")
  }
