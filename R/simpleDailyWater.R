...
simpleDailyWater <- function(CRHMoutput, vars='all', prjFile='', basinMean=TRUE,
  ...
  if (REW_group_count > 0){
   ...
   parsed_variables <- as.numeric(stringr::str_split(variables$variable_name,
                                                      stringr::fixed('@'),2))
  ...
  # now aggregate all variables
    ...
    # update units
    variables$units[area_vars] <- stringr::str_replace(variables$units[area_vars],
  ...
  comment <- paste('simpleDailyWater CRHMoutput:',output_name ,
                   ...
                   ' summarize:', summarize, ' units:', units, sep='')
