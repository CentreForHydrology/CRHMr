...
#'
#' @description Many time series have missing rows. This function finds missing rows, and adds rows with the appropriate datetime values and \code{NA_real_} values for all variables. Once the missing values have been added, you can use \code{\link{interpolate}} or \code{\link{impute}} to infill them.
#' @param obs Required. A \pkg{CRHMr} data frame containing obs values.
#' @param quiet Optional. Suppresses display of messages, except for errors. If you are calling this function in an \R script, you will usually leave quiet=\code{TRUE} (i.e. the default). If you are working interactively, you will probably want to set quiet=\code{FALSE}.
