#' Quantile-quantile plot by season
#'
...
#' @param seasons Required. A vector containing the names of the seasons. Can be any length and can contain any values.
#' @param startDayNums Required. A vector containing the starting day number of each season.
...
#' print(p)
seasonalQQplot <- function(primaryCRHM, primaryCol=1,
                          secondaryCRHM, secondaryCol=1,
                          seasons = c("spring", "rest of year"),
                          startDaynums = c(90, 120),
  ...
  if (length(seasons) != length(startDaynums)) {
    cat("Error: differing numbers of seasons and startDaynums\n")
    return(FALSE)
  }


  primary_variable_name <- names(primaryCRHM)[primaryCol]
  primary_rh_loc_num <- length(grep("rh", tolower(primary_variable_name),
                                    fixed = TRUE))
  secondary_variable_name <- names(secondaryCRHM)[secondaryCol]
  secondary_rh_loc_num <- length(grep("rh", tolower(secondary_variable_name),
                                      fixed = TRUE))
  ...
  if (samePeriod) {
    ...
    merged <- merge(primaryCRHM_selected, secondaryCRHM_selected,
                    by = 'datetime',
                    all = TRUE)
    ...
    # get season
    merged$dayofyear <- as.numeric(format(merged$datetime, format = "%j"))
    merged$season[merged$dayofyear >= startDaynums[i]] <- seasons[i]

    season_count <- length(seasons)
    for (i in 1:season_count) {
      season <- seasons[i]
      seasonal <- merged[merged$season == season,]
      qqvals <- qqplotValues(seasonal[,2], seasonal[,3])
      if (i == 1)
        plotvals <- data.frame(season, qqvals)
      else
        plotvals <- rbind(plotvals, data.frame(season, qqvals))
    }
  }
  else{

    primaryCRHM_selected$dayofyear <- as.numeric(format(primaryCRHM_selected$datetime, format = "%j"))
    primaryCRHM_selected$season[primaryCRHM_selected$dayofyear >= startDaynums[i]] <- seasons[i]

    season_count <- length(seasons)
    for (i in 1:season_count) {
      primary_monthly <- primaryCRHM_selected[primaryCRHM_selected$season == seasons[i] ,]
      secondary_monthly <- secondaryCRHM_selected[secondaryCRHM_selected$month == seasons[i],]
      qqvals <- qqplotValues(primary_monthly[,2], secondary_monthly[,2])
      if (i == 1)
        plotvals <- data.frame(seasons[i], qqvals)
      else
        plotvals <- rbind(plotvals, data.frame(seasons[i], qqvals))
    }
  }
