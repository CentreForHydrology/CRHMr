...
#'
#' @description Missing (\code{NA}) values in a \pkg{CRHMr} obs data frame are filled by linear or spline interpolation. The user can set the maximum gap length (in time steps) allowed to be filled. Note that this function will \emph{NOT} interpolate RH or precipitation values.
#' @param obs Required. The \pkg{CRHMr} data frame of obs values.
#' @param varcols Optional. A vector containing the columns to be imputed in the obs data frame, not including the datetime. If not specified, defaults to the first column.
#' @param methods Optional. A vector containing the methods to be used for interpolation for each of the variables. Currently supported methods are \option{linear} and \option{spline}. The default is to use linear interpolation. If fewer methods than columns are specified, the methods are recycled.
...
#' @return If successful, returns a modified version of the \code{obs} data frame. The missing values in the specified data frame are replaced by interpolated values. If unsuccessful, returns the value \code{FALSE}.
#' @author Kevin Shook
...
function(obs, varcols=1, methods='linear', maxlength=5, 
  ...
  action <- 'interpolate'
  result <- writeChangeLogFile(action, original.data.info, new.data.info, output.type,
