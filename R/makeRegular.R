...
#'
#' @return If successful, returns a modified version of the \code{obs} data frame containing the adjusted datetime and all of the variables. If unsuccessful, returns \code{FALSE}.
#' @author Kevin Shook
#' 
#' @export
#'
...
makeRegular <- function(obs, timezone='', quiet=TRUE, logfile=''){
  ...
  if (timezone == ''){
    cat('Error: timezone must be specified\n')
    return(FALSE)
  }
