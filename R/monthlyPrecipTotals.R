...
#'
#' @description Calculates mean total precipitation for each calendar month in the observation data frame. Optionally plots the values. Can work with \code{p} and/or \code{ppt} values.
#' @param obs Required. A \pkg{CRHMr} data frame containing \code{p} and/or \code{ppt} values..
#' @param precipCols Optional. A vector containing the columns to be analyzed, not including the \code{datetime} column. If not specified, all precipitation columns are used.
#' @param plot Optional. If \code{TRUE}, which is the default, a \pkg{ggplot2} object of the monthly mean precipitations is returned. If \code{FALSE}, the values are returned as a data frame.
#' @param logfile Optional. Name of the file to be used for logging the action. Normally not used.
#' @return If successful, returns either a data frame or a \pkg{ggplot2} object of the monthly mean precipitations. If unsuccessful, returns \code{FALSE}.
#' @author Kevin Shook
...
monthlyPrecipTotals <- function(obs, precipCols=0, plot=TRUE, logfile='') {
  ...
  ppt_cols <- which(stringr::str_detect(precips_names,
                                        stringr::fixed('ppt.', ignore_case=TRUE)))
  p_cols <- which(stringr::str_detect(precips_names,
                                      stringr::fixed('p.', ignore_case=TRUE)))

  ppt_cols_count <- length(ppt_cols)
  p_cols_count <- length(p_cols)

  if ((ppt_cols_count <= 0) & (p_cols_count <= 0)){
    cat('Missing precip values in obs data frame\n')
    return(FALSE)
  }
  ...
  if (ppt_cols_count > 0){
    ...
    dt <- timestep.hours(precips$datetime[2], precips$datetime[1])
    ...
    ppt_monthly <- aggregate(ppt_daily[,ppt_daily_cols], by=list(ppt_daily$month_year,
                                                             ppt_daily$month), FUN='sum')
  ...
  all_melted <- reshape2::melt(all_monthly_mean, id.vars='month' )
  ...
  if (plot) {
    ...
    p_plot <- ggplot2::ggplot(all_melted, ggplot2::aes(month, value)) +
      ggplot2:: geom_bar(stat="identity") +
      ggplot2::ylab('Mean monthly precipitation (mm)') +
      ggplot2::facet_wrap(~ variable) + ggplot2::xlab('')
  }

  comment <- paste('monthlyPrecipDistribution obs:', obsName,
                   ' precip_variables:',
                   stringr::str_c(precip_variable_names, collapse=','),
                   sep='')
