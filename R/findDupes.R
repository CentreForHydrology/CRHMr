...
#'
#' @description Finds duplicate datetime values. All of the duplicate values are written
#' to a .csv file. Many time series, especially from Environment Canada, may contain
#' duplicate datetimes.
#' @param obs Required. A \pkg{CRHMr} data frame containing obs values.
#' @param dupefile Optional. The name of the output file. If omitted the dupe file will
#' be the name of the obs data frame, followed by \option{_dupedatetimes.csv}.
...
#' Normally not used.
#' @return If there are duplicate datetimes, returns \code{TRUE}. If there are no
#' duplicates, returns \code{FALSE}.
#' @author Kevin Shook
#' @note If quiet=\code{FALSE}, the function gives a list of the duplicate datetimes.
#' Use this function before removing duplicates and/or interpolation and imputation.
#' Most importantly, use this function before writing values to an obs file. Duplicate
#' datetime values in an obs file will prevent CRHM from executing.
#' @seealso \code{\link{findGaps}} \code{\link{interpolate}} \code{\link{impute}}
#' \code{\link{writeObsFile}}
#' @examples
