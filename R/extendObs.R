...
#'
#' @description This function extends a \pkg{CRHMr} obs data frame to include the specified dates. All added values are set to be \code{NA_real_}. The purpose of this function is to create locations for data that can be infilled or imputed from another data set.
#' @param obs Required. The \pkg{CRHMr} obs data frame to be extended.
#' @param startDate Optional. The starting date for the obs data frame. Can either be a year as a number (e.g. \code{1995}) or a date in \option{Y-m-d} format, i.e. \option{1995-06-01}. Either the startDate or the endDate, or both, must be specified.
#' @param endDate Optional. The ending date for the obs data frame.  Can either be a year as a number (e.g. \code{1995}) or a date in \option{Y-m-d} format, i.e. \option{1995-07-01}.
...
#' @return If successful, returns a modified version of the obs data frame. The missing values in the specified data frame are replaced by \code{NA_real_} values. If unsuccessful, returns the value \code{FALSE}.
#' @author Kevin Shook
#' @examples
...
extendObs <- function(obs, startDate='', endDate='', timezone='', quiet=TRUE, logfile=''){
  ...
  if ((startDate == '') & (endDate == '')){
    cat('Error: start date and/or end date must be specified\n')
    return(FALSE)    
  }
  if (timezone == ''){
