...
#' @param start.date Required. Starting date, in the format \option{d/m/Y}, \option{d-m-Y}, \option{d B Y}, \option{d b Y}, \option{d-B-Y}, \option{d-b-Y}, \option{B d, Y}, or \option{b d, Y}, where \option{Y} = year, \option{m} = month number, \option{B} = capitalized month name, \option{b} = lower case month name, \option{d} = day number.
...
#' @param timestep Optional. Time interval between values in hours. The default value is 1 hour.
...
function(start.date, end.date, timestep=1, 
  ...
  date.formats <- c('d/m/Y','d-m-Y','d B Y','d b Y','d-B-Y','d-b-Y' ,
                    'B d, Y', 'b d, Y')
  ...
  if ((timestep==24) | (timestep=='daily') | (timestep=='daily')){
    ...
    time.seq <- seq(from=start.time, to=end.time, by=3600)
  }
