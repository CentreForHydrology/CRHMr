...
#' @name distributeQsi
#' @description Downscales incoming shortwave radiation to shorter time intervals, based on the extra-terrestrial radiation. The mean atmospheric transmittance is computed from the ratio of the incoming shortwave to the extra-terrestrial radiation over the original time intervals. The extra-terrestrial radiation over the shorter time intervals is then multipled by the mean transmittance values to calculate the shorter interval incoming shortwave radiation.
#' @param QsiObs Required. A \pkg{CRHMr} obs data frame containing incoming shortwave radiation in in W/m\eqn{^2}{^2}.
#' @param QsiColnum Optional. The number of the columns (not including the datetime) containing the incoming shortwave values. Default is \code{1}.
#' @param latitude Required. The latitude for which values are to be calculated.
#' @param sunTimeOffset Optional. The offset (in hours) is added to the solar time to convert it to local time. The default value \option{2} shifts the daily peak to occur at 2pm.
...
#' @param details Optional. If \code{TRUE} then the calculated atmospheric transmittance and extra-terrestrial radiation (\code{So_h_W}) will be returnes along with the downscaled Qsi. If \code{FALSE} (the default) then only the Qsi values and the datetime are returned.
#' @param quiet Optional. Suppresses display of messages, except for errors. If you are calling this function in an \R script, you will usually leave \code{quiet=TRUE} (i.e. the default). If you are working interactively, you will probably want to set \code{quiet=FALSE}.
#' @param logfile Optional. Name of the file to be used for logging the action. Normally not used 
#' @return If successful, returns an obs data frame containing the downscaled incoming shortwave radiation (Qsi) in W/m\eqn{^2}{^2} and, optionally, the transmittance and extra-terrestrial solar radiation (\code{So_h_W}). If unsuccessful, returns the value \code{FALSE}.
#' @author Kevin Shook
...
distributeQsi <- function(QsiObs, QsiColnum=1, latitude='', sunTimeOffset=2, timeStep=1,
  ...
  # check parameters
  if (timeStep < 1){
    cat('Error: time step must be specified and >= 1 hour\n')
    return(FALSE)
  }
  if (nrow(QsiObs) == 0){
    cat('Error: missing Qsi values\n')
    return(FALSE)
  }
  if (latitude == ''){
  ...
  if(stringr::str_to_lower(solarMethod) == 'simplemaxsolar'){
    ...
    So_h_W <- EcoHydRology::PotSolarInst(Jday, hour, lat=latitude, latUnits='degrees',
                                 SolarNoon=(12+sunTimeOffset), units='Wm2')
    downscaleMax <- data.frame(downscaledDatetime, So_h_W)
    names(downscaleMax)[1] <- 'datetime'
  }
  else{
    cat('Error: missing or incorrect solarMethod\n')
    return(FALSE)
  }
