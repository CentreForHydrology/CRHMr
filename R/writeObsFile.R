...
#'
#' @description Writes a CRHM dataframe to an obs file. It will not allow ea values - they must be converted to RH values first. The CRHM filter command to convert RH values to ea is added automatically to the header of the .obs file.
#' @param obs Required. A \pkg{CRHMr} obs data frame.
#' @param obsfile Required. Name for file to hold observations.
#' @param comment Optional. A comment to be added to the header of the .obs file. If omitted, the header will consist of the name of the obs data frame followed by \option{created by CRHMr function writeObsFile}.
...
writeObsFile <- function(obs, obsfile='',  comment='', quiet=TRUE, logfile=''){
  ...
  var.units[var.names == 'p'] <- '(mm)'
  var.units[var.names == 'ppt'] <- '(mm)'
  var.units[var.names == 'ea'] <- '(kPa)'
  var.units[tolower(var.names) == 'qsi'] <- '(W/m2)'
  var.units[tolower(var.names) == 'qso'] <- '(W/m2)'
  var.units[tolower(var.names) == 'qsn'] <- '(W/m2)'
  var.units[tolower(var.names) == 'qli'] <- '(W/m2)'
  var.units[tolower(var.names) == 'qlo'] <- '(W/m2)'
  var.units[tolower(var.names) == 'qln'] <- '(W/m2)'
  var.units[tolower(var.names) == 'qn'] <- '(W/m2)'
  ...
  cat('################','\t',colnames, eol.val, sep='', file=obsfile, append=TRUE)
  ...
  obs$datetime <- format(obs$datetime, format='%Y %m %d %H %M')
